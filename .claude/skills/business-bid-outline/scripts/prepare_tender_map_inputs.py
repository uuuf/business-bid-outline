...
def matched_checklist_items(text, checklist):
    ...
    for item in checklist:
        ...
        hit_terms = [term for term in terms if term and normalize(term) in normalized_text]
        if hit_terms:
            matches.append({"title": item["title"], "matched_terms": sorted(set(hit_terms))})
    return matches
...
def zone_anchor_block(block, match):
    ...
    if is_cross_reference(text, match["title"]):
        return False
    title_hit = normalize(match["title"]) in normalize(text)
    if title_hit:
        return True
    # Avoid turning every numbered material requirement into a zone just
    # because it mentions a broad hint such as "业绩" or "证明材料".
    return block.get("heading_level") is not None and len(text) <= 100
...
def build_checklist_hits(blocks, tables, zones, checklist):
    ...
    for item in checklist:
        ...
        terms = [item["title"], *item.get("scan_hints", [])]
        for block in blocks:
            matched_terms = [term for term in terms if term and normalize(term) in normalize(block.get("text", ""))]
            if matched_terms:
                hit = {"block_id": block["block_id"], "matched_terms": sorted(set(matched_terms))}
                if block.get("table_id"):
                    hit["table_id"] = block["table_id"]
                block_hits.append(hit)
        for table in tables:
            table_text = "\n".join(row.get("row_text", "") for row in table.get("rows", []))
            table_match_text = "\n".join([table.get("nearby_heading", ""), table.get("nearby_caption", ""), table_text])
            matched_terms = [term for term in terms if term and normalize(term) in normalize(table_match_text)]
            if matched_terms:
                table_hits.append({"table_id": table["table_id"], "matched_terms": sorted(set(matched_terms))})
        for zone in zones:
